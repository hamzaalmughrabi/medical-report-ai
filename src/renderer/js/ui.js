...
export const ui = {
    ...
    setActiveLink: (pageId) => {
        ...
        allNavs.forEach(el => {
            el.classList.remove(
                "active", 
                "bg-primary/5", "bg-primary/10", "bg-slate-50",
                "text-primary", "font-semibold", "font-bold",
                "border-l-4", "border-primary", "rounded-l-none"
            );
            
            const parentLi = el.closest("li");
            if (parentLi) {
                parentLi.classList.remove("bg-primary/10", "bg-slate-100");
                parentLi.querySelectorAll(".status-indicator").forEach(i => i.remove());
            }
        });

        const activeEls = Array.from(allNavs).filter(el => el.dataset.page === pageId);
        activeEls.forEach(el => {
            el.classList.add("active");
            
            // Premium Sidebar Active Styling
