...
def _coerce_report_dict(raw: Any) -> Dict[str, Any]:
    ...
    if isinstance(raw, str):
        ...
        try:
            ...
            # Treat it as a raw textual report; downstream hydration will parse it.
...
def _find_value_recursive(data: Any, search_keys: Sequence[str]):
    """
    Recursively search nested dict/list structures for the first matching key
    (case-insensitive). This helps when upstream JSON embeds patient fields in
    unexpected sub-objects, e.g., {"patient": {"name": "John"}} or deeply nested
    metadata blobs.
    """

    if isinstance(data, dict):
        for key in search_keys:
            value = _get_value_case_insensitive(data, key)
            if value not in (None, ""):
                return value

        for value in data.values():
            found = _find_value_recursive(value, search_keys)
            if found not in (None, ""):
                return found

    elif isinstance(data, list):
        for item in data:
            found = _find_value_recursive(item, search_keys)
            if found not in (None, ""):
                return found
...
def _extract_first(report: Dict[str, Any], keys: Sequence[str], container_keys: Sequence[str] | None = None):
    ...
    # Deep fallback: search the entire structure recursively so that deeply
    # nested patient details (common in some LLM outputs) are still surfaced.
    deep_val = _find_value_recursive(report, keys)
    if deep_val not in (None, ""):
        return deep_val
    return None
